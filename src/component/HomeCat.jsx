    ...
    return (
        <div className="pb-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {friends.map((friend) => (
                    <div key={friend.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center text-center space-y-4 hover:shadow-lg transition">
                        ...
                        <div className="flex flex-wrap gap-2 justify-center">
                            {friend.tags.map((tag, index) => (
                                <span key={index} className="text-[10px] font-semibold bg-green-200 text-[#244D3F] px-3 py-1 rounded-full uppercase tracking-wider">
                                    {tag}
                                </span>
                            ))}
                        </div>
